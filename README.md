...
- /(?:https?:\/\/www.|https?:\/\/)([^\/]+)/ を正規表現として使えばhttps://以下のドメインを取得できます</br></br>  
...
不具合の修正（検索結果によって表示の仕方が異なりエラーが生じた箇所に対応するなど。）  
削除した件数を表示させる  
Google検索以外の検索エンジンや別プラットフォーム版を作ってみる  
アプリをアンインストールしてもデータが削除されないようにしておく
  
などなど、オリジナルの機能を追加してみようっっっ！！！！  
  
  
良さそうな機能はディレクトリFinishedProduceへ修正したのちプルリクをもらえれば反映します。